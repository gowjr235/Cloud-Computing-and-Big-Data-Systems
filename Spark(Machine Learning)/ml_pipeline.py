...
# MAGIC * Heysem Kaya, Pinar Tüfekci and Fikret S. Gürgen: [Local and Global Learning Methods for Predicting Power of a Combined Gas & Steam Turbine](http://psrcentre.org/images/extraimages/70.%20312595.pdf), Proceedings of the International Conference on Emerging Trends in Computer and Electronics Engineering ICETCEE 2012, pp. 13-18 (Mar. 2012, Dubai).
# MAGIC 
...
# MAGIC %md
# MAGIC First, let's register our DataFrame as an SQL table named `plant_dataset`.  Because you may run this lab multiple times, we'll take the precaution of removing any existing tables first.
# MAGIC 
...
# MAGIC %md
# MAGIC Now that our DataFrame exists as a SQL table, we can explore it using SQL commands. This [mini tutorial](https://tutorialzine.com/2016/01/learn-sql-in-20-minutes) could help refeshing the SQL knowledge, and the top three sections therein are enough for this lab.
# MAGIC 
...
# MAGIC %sql
# MAGIC -- TODO: Replace <FILL_IN> with the appropriate SQL command.
# MAGIC select PE as PowerOutput, RH as RelativeHumidity from plant_dataset
...
(split15DF, split85DF) = datasetDF.randomSplit([0.15, 0.85],1900009193)
...
# TEST
Test.assertEquals(trainingSetDF.count(), 40567, "Incorrect size for training data set")
Test.assertEquals(testSetDF.count(), 7273, "Incorrect size for test data set")
...
# MAGIC %md
# MAGIC We can use SQL to explore the `Plant_RMSE_Evaluation` table. First let's look at at the table using a SQL SELECT statement.
# MAGIC 
# MAGIC ### Exercise 6(i)
# MAGIC 
# MAGIC Run the next cell and ensure that you understand what's going on.
...
# MAGIC %md
# MAGIC From the pie chart, we can see that 69% of our test data predictions are within 1 RMSE of the actual values, and 97% (69% + 28%) of our test data predictions are within 2 RMSE. So the model is pretty decent. Let's see if we can tune the model to improve it further.
...
# TEST
Test.assertEquals(round(rmse, 2), 4.56, "Incorrect value for rmse")
Test.assertEquals(round(rmseNew, 2), 4.55, "Incorrect value for rmseNew")
...
# MAGIC %md
# MAGIC So tuned linear regression model improves a little bit over our initial model. Let's look at the regularization parameter that the [CrossValidator](https://spark.apache.org/docs/1.6.2/ml-tuning.html#cross-validation) has selected.
# MAGIC 
...
# TEST
Test.assertEquals(round(rmseDT, 2), 5.07, "Incorrect value for rmseDT")
...
# MAGIC   - Set the name of the label column to "PE"
# MAGIC   - Set the name of the features column to "features"
# MAGIC   - Set the random number generator seed to 190088121
...
# TEST
Test.assertEquals(round(rmseRF, 2), 3.49, "Incorrect value for rmseRF")
