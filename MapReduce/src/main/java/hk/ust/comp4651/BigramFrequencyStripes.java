...
public class BigramFrequencyStripes extends Configured implements Tool {
	...
	private static final IntWritable excess = new IntWritable();
	...
	private static class MyReducer extends
		...
		public void reduce(Text key,
			...
			Set<String> hashMapKeys = SUM_STRIPES.keySet();
			for(String hashMapKey : hashMapKeys){
				BIGRAM.set(key.toString(), hashMapKey);
				if(hashMapKey.equals("")){
					//(a,*)
					excess.set(SUM_STRIPES.get(hashMapKey));
					FREQ.set(excess.get());
					context.write(BIGRAM, FREQ);
				} else {
					//(a,x)
					FREQ.set(SUM_STRIPES.get(hashMapKey) / (float) excess.get());
					context.write(BIGRAM, FREQ);
				}
			}
			SUM_STRIPES.clear();
