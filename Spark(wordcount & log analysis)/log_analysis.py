...
# MAGIC #### **Part 1: Apache Web Server Log file format**
# MAGIC 
# MAGIC The log files that we use for this assignment are in the [Apache Common Log Format (CLF)](https://httpd.apache.org/docs/). The log file entries produced in CLF will look something like this:
...
# MAGIC  
# MAGIC To create the primary RDD that we'll use in the rest of this assignment, we first load the text file using [`sc.textfile(logFile)`](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.SparkContext.textFile.html) to convert each line of the file into an element in an RDD.
# MAGIC 
# MAGIC Next, we use [`map(parseApacheLogLine)`](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.RDD.map.html) to apply the parse function to each element (that is, a line from the log file) in the RDD and turn each line into a pair [`Row` object](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.sql.Row.html).
# MAGIC 
...
# MAGIC %md
# MAGIC **(3a) Exercise: Top Five Error Endpoints**
# MAGIC 
...
uniqueHosts = hosts.reduceByKey(lambda log, y: log + y)
...
# MAGIC %md
# MAGIC **(3c) Exercise: Number of Unique Hourly Hosts**
# MAGIC 
...
hourToHostPairTuple = access_logs.map(lambda log: (log.date_time.hour, log.host)).distinct()

hourGroupedHosts = hourToHostPairTuple.groupByKey()

hourHostCount = hourGroupedHosts.map(lambda hourTuple: (hourTuple[0], len(hourTuple[1])))

hourlyHosts = hourHostCount.sortBy(lambda x:x[0]).cache()
...
# MAGIC %md
# MAGIC **(3d) Exercise: Visualizing the Number of Unique Hourly Hosts**
# MAGIC 
...
# MAGIC 
# MAGIC How could you convert a RDD into a list? See the [`collect()` method](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.RDD.collect.html).
...
# MAGIC %md
# MAGIC **(3e) Exercise: Average Number of Hourly Requests per Host**
# MAGIC 
...
groupedByHour = hourAndHostTuple.groupByKey()

sortedByHour = groupedByHour.join(hourlyHosts).sortBy(lambda x:x[0])

avgHourlyReqPerHost = sortedByHour.map(lambda x:(x[0], len(x[1][0])/x[1][1])).cache()
...
# MAGIC %md
# MAGIC **(3f) Exercise: Visualizing the Average Hourly Requests per Unique Host**
# MAGIC 
...
badEndpointsCountPairTuple = badRecords.map(lambda record: (record.endpoint, 1))

badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(lambda a, b: a+b)

badEndpointsTop10 = badEndpointsSum.sortBy(lambda x:-x[1]).take(10)
